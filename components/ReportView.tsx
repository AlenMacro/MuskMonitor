...
export const ReportView: React.FC<ReportViewProps> = ({ report, isWriting = false }) => {
  ...
  // Smart Link Logic
  const getSmartTweetLink = () => {
    if (!report.tweet) return "https://x.com/elonmusk";
    
    const { url, text } = report.tweet;
    
    // 1. If we have a specific status link, trust it
    if (url && (url.includes('/status/') || url.includes('/statuse/'))) {
        return url;
    }

    // 2. If no specific link, construct a search query for this tweet from Elon
    // We limit text length to ensure URL validity
    const safeText = text.substring(0, 100).replace(/[^\w\s]/gi, ''); // clean special chars
    const encodedText = encodeURIComponent(`from:elonmusk ${safeText}`);
    return `https://x.com/search?q=${encodedText}&src=typed_query&f=live`;
  };

  const tweetLink = getSmartTweetLink();
    ...
    <div className="space-y-8">
      ...
      {hasTweet && (
        <div className="bg-black border border-[#2f3336] rounded-2xl overflow-hidden animate-fade-in-up delay-100 max-w-2xl mx-auto md:mx-0 shadow-2xl hover:bg-neutral-900/40 transition-colors">
          <a href={tweetLink} target="_blank" rel="noopener noreferrer" className="block p-6 flex gap-4 no-underline group/card">
             {/* Fake Avatar */}
